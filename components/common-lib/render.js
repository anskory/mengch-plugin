...
const plugin = 'mengch-plugin'
const _path = process.cwd()

export default async function (path, params, cfg) {
  let [app, tpl] = path.split('/')
  let { e } = cfg
  let layoutPath = process.cwd() + `/plugins/${plugin}/resources/common/layout/`
  ...
  let data = {
    ...
    _tpl_path: process.cwd() + `/plugins/${plugin}/resources/common/tpl/`,
    ...
    sys: {
      ...
      copyright: `Created By Yunzai-Bot<span class="version">${Version.yunzai}</span> & Mengch-Plugin<span class="version">${Version.version}</span>`
    }
  }
